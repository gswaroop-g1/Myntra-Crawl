...
def process_page(page_id)
	url = "http://www.myntra.com/women-tshirts/page/#{page_id}"
	csv_file = File.open("./women-tops-tunics.csv", "a")
	begin
	...
	products.each do |li|
		...
		begin
			begin
				...
				flag = true if(!product_url.match(/Tshirt/))
				# next if(product_url.match(/Tops/))
			rescue Exception => e
